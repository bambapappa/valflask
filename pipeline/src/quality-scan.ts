 ...
 */
const MILJARDENHETER = /^(miljard|mdkr)/i;
...
const BARE_BASE_PATTERNS = [
  ...
  // spannets lågvärde.
  new RegExp(`\\bbase?(?:belopp|fall|nivå|antagande)?(?:et|en|t)?\\s*(?::|=|är|blir|sätts till|läggs på|sätts)?\\s*${UNGEFAR}(${NUM_SRC})(?!\\d|\\s*(?:%|procent))`, "i"),
  ...
  new RegExp(`\\bbase?(?:belopp|fall|nivå|antagande)?(?:et|en|t)?\\s*(?::|=)?\\s*${UNGEFAR}(?:${NUM_SRC})\\s*(?:%|procent)\\w*\\s*(?:[≈=→]|ger|blir|motsvarar)\\s*${UNGEFAR}(${NUM_SRC})`, "i"),
 ...
 */
const STORHETSENHET = /^\s*(GWh|TWh|kWh|MWh|ton|hektar|ha|kontor|platser|personer|elever|studenter|årsarbetskraft\w*|tjänster)\b/iu;
...
function isOperand(tail: string): boolean {
  ...
  return STORHETSENHET.test(tail) && /^.{0,20}?[\u2192\u2248]/u.test(led);
}
...
function sistaTotalbeloppet(text: string): number | null {
  const traffar = [...text.matchAll(new RegExp(`(${NUM_SRC})\\s*(${ENHET_ALT})`, "giu"))].filter(
    (m) => !arStyckpris(text.slice((m.index ?? 0) + m[0].length)),
  );
