...
import { forberedUtrakningsforslag, type FrystUtrakningsforslag } from "./utrakningsforslag.ts";
import { byggUtrakningsunderlag, skapaSakprovning, sakprovningsBeredskap, type Sakprovning, type Sakreferens } from "./sakprovning.ts";
import { ORSAKKODER, type Orsakkod } from "./orsakkoder.ts";
import { computeDataHash } from "./publish.ts";
import { svenskDag } from "./dagen.ts";
import type { Utrakningsrad } from "./utrakningsbyte.ts";
import type { PromiseEntry } from "./loftesforslag.ts";

export const UTRAKNINGSFILER = ["promises.json", "rattelser.json", "changelog.json"] as const;
export interface Utrakningsindata { rader: Utrakningsrad[]; material: Record<string, Sakreferens[]>; varfor: string; orsak: Orsakkod }
export interface Utrakningspaket {
  version: "utrakningspaket/1";
  tidpunkt: string;
  indata: Utrakningsindata;
  forslag: FrystUtrakningsforslag[];
  provningar: Sakprovning[];
  filer: Filpaket;
}
export function utrakningspakethash(p: Utrakningspaket): string {
...
export function forberedUtrakningspaket(indata: Utrakningsindata, fore: Fillage, nu: Date): Utrakningspaket {
  if (Object.keys(fore).sort().join() !== [...UTRAKNINGSFILER].sort().join()) throw new Error("Fel filuppsättning");
  ...
  const forslag = indata.rader.map((r) => forberedUtrakningsforslag(r, loften, nu));
  const provningar = forslag.map((f) => skapaSakprovning(byggUtrakningsunderlag(f, loften, indata.material[f.rad.id] ?? [], forslag.filter((andra) => andra.rad.id !== f.rad.id))));
  ...
  const rattelser = [...lista(fore, "rattelser.json"), {
    ...
    what: `${forslag.length} löften har fått uträkningen omskriven. Beloppen är oförändrade. ` + indata.rader.map((r) => r.skal.replace(/\.?$/u, ".")).join(" "),
    why: indata.varfor, orsak: indata.orsak, commit: "0000000",
  }];
  const changelog = [...lista(fore, "changelog.json"), {
    run_id: `utrakning-byt-${datum}`, added: [], updated: indata.rader.map((r) => r.id), retracted: [],
    data_hash: computeDataHash(nya), timestamp: nu.toISOString(),
  }];
  const json = (v: unknown): string => JSON.stringify(v, null, 2) + "\n";
  return { version: "utrakningspaket/1", tidpunkt: nu.toISOString(), indata: structuredClone(indata), forslag, provningar,
...
export function kontrolleraUtrakningspaket(p: Utrakningspaket, aktuellt: Fillage): void {
  if (kanoniskJson(p.filer.fore) !== kanoniskJson(aktuellt)) throw new Error("Paketets föreläge har ändrats");
  const nytt = forberedUtrakningspaket(p.indata, aktuellt, new Date(p.tidpunkt));
...
export function verkstallUtrakningspaket(dir: string, p: Utrakningspaket, beslutetsHash: string): void {
  if (!/^[0-9a-f]{64}$/u.test(beslutetsHash) || utrakningspakethash(p) !== beslutetsHash) throw new Error("Beslutet gäller inte hela paketet");
  kontrolleraUtrakningspaket(p, lasFillage(dir, UTRAKNINGSFILER));
