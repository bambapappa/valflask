...
import { avvisningsforslagshash, type Avvisningspaket } from "../src/avvisningspaket.ts";
...
if (cmd?.action !== "approve-package" && cmd?.action !== "reject-package") process.exit(0);
const typ = cmd.action === "reject-package" ? "avvisningspaket" : "godkannandepaket";
...
try {
  ...
  const name = privatPaketnamn(issue, cmd.hash, typ);
  ...
  const paket = JSON.parse(bytes.toString()) as Godkannandepaket | Avvisningspaket;
  if (paket.beslut !== null || (typ === "godkannandepaket"
      ? paket.version !== "godkannandepaket/1" || godkannandeforslagshash(paket as Godkannandepaket) !== cmd.hash
      : paket.version !== "avvisningspaket/1" || avvisningsforslagshash(paket as Avvisningspaket) !== cmd.hash)) throw new Error("Fel paket");
  ...
  console.error("Privat beslutspaket kunde inte verifieras. Kontrollera producentkörning, läsbehörighet och paketets identitet.");
  process.exit(1);
}
