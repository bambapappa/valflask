...
test("privat förberedelse börjar oavgjort, vägrar överskrivning och stoppar manipulerat manifest", async () => {
  ...
  try {
    ...
    writeFileSync(pf, JSON.stringify(p)); writeFileSync(material, JSON.stringify({ poster: {}, helhet: [] }));
    ...
    assert.ok([prov.helhet, ...prov.poster].every((r: any) => r.bedomare === null && r.bedomningar.every((b: any) => b.utfall === "oavgjort")));
    ...
    const utanHelhet = join(dir, "utan-helhet.json");
    assert.equal(run("forbered", pf, mf, material, utanHelhet).status, 1);
    assert.equal(existsSync(utanHelhet), false);
    writeFileSync(material, JSON.stringify({ poster: {}, helhet: [] }));
