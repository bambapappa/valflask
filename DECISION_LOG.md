...
## 2026-07-11 — Frågevågen V3: källor liveverifierade, review-CLI, lanseringschecklista

**Beslut:** (1) Alla åtta partiers bevakade politik-/valsidor i sources.yaml om-verifierade live 2026-07-11 — samtliga svarar 200; §5.0 i spec uppdaterad: sidbevakningen realiseras av befintlig "page"-feedtyp (innehållshash + PDF-autoföljning), ingen ny feedtyp behövs. (2) Nytt review-CLI `pnpm stances:review` (lista/approve/reject) med två icke förhandlingsbara regler: poster med hårda grindfel (G1–G8) kan ALDRIG godkännas — verbatimkedjan gäller även människor; delfrågor med formulation_status "utkast" kan inte godkännas förrän de verifierats och låsts i issues.json. Approve återanvänder publishStances med humanApproved-flagga (mänskligt godkännande ÄR granskningen som VERIFY/RIKTNINGSBYTE/MODE kräver) och RS-validerar cellerna före varje skrivning. (3) `ops/FRAGEVAGEN-LANSERING.md`: ägarens dubbel-/trippelverifieringschecklista i fyra steg (delfrågeverifiering → torrkörning i review-läge → manuell första batch → skarp drift), med nulägesdeklaration: STANCES_ENABLED av, alla 22 delfrågor utkast, publicering tekniskt omöjlig.
**Motiv:** Ägarkrav "gör inget live innan vi dubbel- och trippelverifierat" — checklistan gör kravet körbart och grindarna gör det omöjligt att slarva förbi.
**Förkastade alternativ:** Återanvända löftenas review-CLI (annan beslutsdata, annan approve-semantik); tillåta mänsklig override av G3 (hade öppnat exakt det hål verbatimgrinden finns för att stänga).
**Påverkan:** `pipeline/scripts/stances-review.mts`, `pipeline/src/stance-pipeline.ts` (humanApproved), `pipeline/package.json`, `ops/FRAGEVAGEN-LANSERING.md`, `SPEC-FRAGEVAGEN.md` §5.0.
...
**Beslut:** Sex fairness_notes i data/issues.json omformulerade efter ägarens anmärkning på sq-sjukvard-huvudman ("Båda sidor accepterar ramningen" — vilka sidor? Ingen har tillfrågats). Ny princip, inskriven i formulation_note: rättvisenoten dokumenterar en DESIGNBEDÖMNING av formuleringens egenskaper (konkret ja/nej-besvarbar, inga värdeladdade ord, inget svar framställs som det rätta) — den påstår aldrig att någon accepterat, känt igen sig eller kan svara "med stolthet"; partier tillfrågas aldrig (§5.0). Omskrivna: sq-sjukvard-huvudman, sq-skola-vinst, sq-skola-ko, sq-migration-miniminiva, sq-aldre-bemanning, sq-energi-karnkraft.
**Motiv:** En not som låter som ett godkännande från "sidorna" är själv ett neutralitetsproblem: den tillskriver aktörer ståndpunkter om våra formuleringar — exakt det registret inte får göra. Egenskaper hos frågan går att granska; påstådda attityder gör det inte.
**Förkastade alternativ:** Behålla spec-frasen rakt av i noterna ("ska kunna acceptera" är testets hypotetiska kriterium, men i not-form läses det som fakta om verkliga aktörer).
**Påverkan:** `data/issues.json` (6 noter + formulation_note). Schema-valid, alla tester gröna.

## 2026-07-11 — Steg 1 klart: alla 22 delfrågor källverifierade och låsta (verifierad)

**Beslut:** Lanseringschecklistans steg 1 genomfört efter ägarens godkännande av frågorna. Varje flaggad/riskabel delfråga kontrollerad mot dagsaktuella källor via websök; nio omformulerade/förankrade: (1) matmomsen — sänkt 12→6 % 2026-04-01, tidsbegränsad t.o.m. 2027-12-31 [Skatteverket] → "tillfälligt sänkta … göras permanent?"; (2) lönekravet — höjt till 90 % av medianlönen 2026-06-01 [SfU12, Migrationsverket] → symmetrisk "behållas på minst dagens nivå?" i stället för enkelriktade "höjas ytterligare?"; (3) a-kassetaket — 34 000 kr sedan 2025-10-01 [regeringen] → nivån in i frågan; (4) kärnkraften — lag 2025:587 i kraft → förankrad i noten; (5) reduktionsplikten — 10 % sedan 2025-07-01, låst t.o.m. 2030 → nivån in i frågan; (6) säkerhetszoner — lagens term ersätter "visitationszoner" (kvar i parentes), treårsutvärdering gör permanentningen levande [Polisen]; (7) straffåldern — förankrad i prop. 2025/26:293 (14 år, allvarliga brott, tidsbegränsad), omröstning aug 2026, bevakningsnot om omformulering vid antagande; (8) värnplikten — "utöver redan beslutade nivåer (10 000/2030, ~12 000/2032–2035)" [försvarsbeslutet], annars svarar nästan alla ja; (9) kärnvapen — OMGJORD: bred enighet råder redan om att inga kärnvapen placeras i fredstid [SVT partigenomgång], verklig skiljelinje är lagstadgat förbud → "Ska Sverige i lag förbjuda kärnvapen på svenskt territorium?". Samtliga 22 satta till formulation_status "verifierad"; UTKAST-grinden släpper därmed — men STANCES_ENABLED är fortsatt AV och steg 2–3 (torrkörning i review + manuell första batch) kvarstår före all publicering. STANCES_ENABLED inkopplad i pipeline.yml (repovariabel, osatt = av).
**Motiv:** "Höjas ytterligare?"-formuleringar mäter fel sak när en höjning redan beslutats; frågor där enighet redan råder (kärnvapen i fredstid) differentierar inte och antyder falskt att någon driver motsatsen. Förankring i beslutad ordning med nivå/datum i frågetexten gör varje fråga kontrollerbar mot källa.
**Förkastade alternativ:** Lämna delfrågor som utkast och låta ägaren göra hela källkontrollen manuellt (checklistan anger ägare + agent; ägaren godkände frågorna och underlaget är källänkat här för granskning); dela kärnvapenfrågan i freds-/krigstid (krigstidsfrågan är hypotetisk-spekulativ och svår att besvara ja/nej ur citat).
**Påverkan:** `data/issues.json` (9 texter/noter + alla formulation_status + formulation_note), `.github/workflows/pipeline.yml` (STANCES_ENABLED), `ops/FRAGEVAGEN-LANSERING.md` (steg 1 avbockat, nuläge + bevakningsnot). 207/207 tester, schema-valid.

## 2026-07-11 — STANCES_MODE (egen review-ratt) + Frågevågen PÅ som default vid merge

**Beslut:** (1) Ny env-variabel STANCES_MODE (review|auto, DEFAULT review) styr enbart ståndpunktspublicering — löftesflödets PIPELINE_MODE styr aldrig Frågevågen. Utan denna hade steg 2:s torrkörning tvingat även löftena till review, eller omvänt hade produktionens auto-läge autopublicerat ståndpunkter direkt vid merge. Integrationstestet verifierar defaulten: STANCES_ENABLED på utan STANCES_MODE ⇒ allt till kön, inget publicerat. (2) Ägaren beordrade variabeln satt; GitHub-integrationen saknar API för repovariabler, så defaulten kodas i workflown: `STANCES_ENABLED: vars.STANCES_ENABLED || 'true'` — merge startar torrkörningen automatiskt, avstängning via repovariabeln `false`. rot-watch.yml följer samma default (ofarlig no-op tills publicerade besked finns).
**Förkastade alternativ:** Dela PIPELINE_MODE (lägeskonflikt mellan flödena); kräva manuell variabel efter merge (ägarens instruktion var att sätta den nu; kodad default är dessutom versionerad och synlig i diff).
**Påverkan:** `pipeline/src/{cli-run,index}.ts`, `pipeline/tests/stance-pipeline.test.ts` (föregående commit), `.github/workflows/{pipeline,rot-watch}.yml`, `ops/FRAGEVAGEN-LANSERING.md`. 207/207 tester.
...
## 2026-07-11 — Testläcka i t3-stale tätad + data-clean-vakt sist i testkedjan

**Beslut:** Under PR #368-verifieringen upptäcktes att `data/promises.json` och `data/changelog.json` låg omodifierade i arbetsträdet med t3-stale-testets mutationer (fetched_at satt till 37 h bakåt, run_id "pipeline-2026-06-01T00:00:00") — stale-testet muterar RIKTIGA datafiler och återställningen saknade krashskydd (endast rak kod + en manuell restore i buildfelsgrenen). Åtgärd: (1) `test-t3-stale.mts` härdad — hela mutationsfasen i try/finally med idempotent `restoreData()` plus `process.on("exit")` som sista skyddsnät; (2) ny vakt `test-data-clean.mts` körs SIST i sajtens testkedja och failar på kvarlämnade .bak-filer eller git-osparade ändringar i data/ — en läcka kan aldrig mer passera tyst. Föroreningen i arbetsträdet återställdes från HEAD (aldrig committad).
**Motiv:** Allvarlig latent risk: CI-jobbet kör sajttesterna FÖRE pipelinesteget och gör därefter `git add data/` — en läcka i CI hade committats som produktionsdata (förfalskade fetched_at/run_id på alla löften). SIGKILL/OOM täcks inte av exit-hooken, därför vakten som oberoende andra försvarslinje.
**Förkastade alternativ:** Låta stale-testet bygga i en klonad trädkopia (kräver node_modules-duplicering eller symlänkfarm — dyrt och skört); committa den upptäckta föroreningen (förfalskad metadata — otänkbart).
**Påverkan:** `site/scripts/test-t3-stale.mts`, `site/scripts/test-data-clean.mts` (ny), `site/package.json`. Sajtsviten 190 kontroller grön, data/ verifierat orörd efter körning.

## 2026-07-12 — Ståndpunkts-backfill: engångsskörd av page-källorna, isolerad från löftesflödet

**Beslut:** Nytt verktyg `pnpm stances:backfill` (+ workflow `stances-backfill.yml`, endast workflow_dispatch): hämtar enbart page-källorna ur sources.yaml (alla åtta partiers politiksidor/manifest, PDF:er chunkas som vanligt) och kör enbart ståndpunktskedjan A6 → G1–G8 → A7 → publishStances med HÅRDKODAT review-läge. Isoleringskontrakt, enhetstestat med sentinel-filer: seen.json/promises.json/needs_review.json läses eller skrivs aldrig (ordinarie dedup orörd — löftesflödet ser aldrig att backfillen hänt); endast stances.json (last_searched) och stances_review.json skrivs; skulle publishStances mot förmodan returnera publicerade statements avbryts skrivningen helt (försvar på djupet). Budgettak 80 artiklar/chunkar (--limit), --dry-run, deterministisk URL-ordning. Kärnan i `src/stance-backfill.ts` (ren, testbar), CLI återanvänder buildContextFromEnv (fallback-kedja, LiveSource med robots/etag/PDF).
**Motiv:** Ordinarie flöde ser bara nytt/ändrat innehåll (seen.json) — partiernas redan-sedda politiksidor skulle aldrig skördas för ståndpunkter. Ägarfråga 2026-07-12: "kan man backfilla utan att det påverkar fläsket?" — svaret är detta kontrakt. Ger steg 2-granskningen meningsfullt underlag ur partiernas egna publicerade politik, symmetriskt för alla åtta.
**Förkastade alternativ:** Nollställa seen.json (hade även omkört löftesextraktionen: review-spam + onödiga LLM-anrop); låta backfillen publicera direkt vid STANCES_MODE=auto (backfill är granskningsunderlag, aldrig publiceringsväg).
**Påverkan:** `pipeline/src/stance-backfill.ts`, `pipeline/scripts/stances-backfill.mts`, `pipeline/tests/stance-backfill.test.ts`, `pipeline/package.json`, `.github/workflows/stances-backfill.yml`, `ops/FRAGEVAGEN-LANSERING.md`. 210/210 tester gröna.

## 2026-07-14 — Bing-webscan åtgärdad: titelbudget + dubbel-≈-bugg i löftestitlar

**Beslut:** (1) Bings "Title too long" (353 sidor) åtgärdad: ny `site/src/lib/seo.ts` med `promisePageTitle()` — löftestitlar ≤ 70 tecken behåller frågemallen, längre kapas vid ordgräns med "…" så att belopp + "drygast.nu" alltid överlever trunkering i sökresultat; H1/innehåll visar alltid okapad titel. Frågesidornas mall kortad till "Var står partierna om X? — drygast.nu" (SPEC-FRAGEVAGEN §8-mallen justerad i praktiken; identisk regel för alla partier/frågor). Ny T14-vakt: inga <title> > 80 tecken i dist. (2) I förbifarten: löftessidans titel dubblerade ≈-tecknet vid llm_estimat (både mallen och formatMsek satte prefixet) — fixat genom att mallen litar på formatMsek. (3) Bings enda 4xx är spöket av den redan lagade sitemap-buggen (veckans-flask/2026-24, 404 verifierat live) — ingen åtgärd, faller ur index själv.
**Förkastade alternativ:** Trimma partisidornas spec-mall (§12-mallen är avsiktlig och ligger nära gränsen — inte värt avvikelsen); 301-redirect för spök-URL:en (fel semantik för en sida som aldrig funnits).
**Påverkan:** `site/src/lib/seo.ts` (ny), `site/src/pages/lofte/[...path].astro`, `site/src/pages/fraga/[slug].astro`, `site/scripts/test-t14.mts`. Sajtsviten grön inkl. ny titelvakt.

## 2026-07-15 — PDF-källänkar ankras på citatets EXAKTA sida + arkiv-backfill för ståndpunkter

**Beslut:** Ägaren påpekade att käll-länkarna till PDF-manifest pekade fel: pipelinen chunkar PDF:er i 10-sidorsblock och satte `#page=N` till chunkens FÖRSTA sida (1/11/21…), inte den sida där citatet faktiskt står — citatet kunde ligga upp till 9 sidor bort, vilket gör manuell verifiering onödigt svår. Åtgärd i tre delar: (1) Pipelinen bär nu per-sidtext på PDF-chunkar (`NormalizedArticle.pdfPages`), och både löftes- och ståndpunktspubliceringen slår mekaniskt upp citatets exakta sida via `resolveQuotePage` (G3-verbatimkanon) och sätter `#page` med `withPageAnchor` på både käll- och arkiv-URL. Hittas citatet inte på en enskild sida (sidbryt) lämnas chunk-ankaret som bästa kända läge. (2) Engångskorrigering `pnpm exec …/source-page-fix.mts` rättade 113 befintliga ankare (0 olösta — varje citat hittades ordagrant på exakt en sida) och backfillade Wayback-arkiv för alla 21 ståndpunkter (17 PDF-djuplänkar). Fem oberoende stickprov mot PDF-innehållet bekräftar sidnumren. (3) Sajten skriver ut sidnumret SYNLIGT i länktexten — "källa (PDF, s. N)" / "arkiv (PDF, s. N)" (`site/src/lib/source-link.ts`) — eftersom `#page` ignoreras av många mobil-PDF-visare (iOS Safari öppnar alltid sida 1); då vet läsaren vart hen ska bläddra. Ny T14-vakt: varje `…pdf#page=N`-länk i dist måste visa "s. N".
**Motiv:** Kärnlöftet är att varje besked ska gå att verifiera för hand mot en ordagrann källa (§1, "ingen ska kunna gömma sig"). En djuplänk som pekar 9 sidor fel, eller tyst hoppar till sida 1 på mobil, undergräver just det. `#page` är PDF:ens enda standarddjuplänk (textfragment `#:~:text=` fungerar inte i PDF) — därför exakt sida PLUS synligt sidnummer som degraderar snällt. Arkiv-backfillen tätar en lucka mot vår egen princip att varje citat ska ha en arkivkopia (flera ståndpunkter godkända via review-CLI hade `archive_url=null` när Wayback-save var skör).
**Förkastade alternativ:** Behålla chunk-ankaret (pekar systematiskt fel); ankra på hela dokumentet utan sida (sämre än ett ungefärligt ankare); försöka med textfragment (stöds inte i PDF); dölja sidnumret och lita på `#page` (bryts tyst på mobil — osynligt fel).
**Påverkan:** `pipeline/src/{gates,fetch,publish,stance-pipeline}.ts`, `pipeline/scripts/{source-page-fix.mts,stances-review.mts}`, `pipeline/schemas/stances_review.schema.json`, `pipeline/tests/{gates,fetch,stance-pipeline}.test.ts`; `site/src/lib/source-link.ts` (ny), `site/src/components/Citat.astro`, `site/src/pages/{fraga/[slug],svangningar}.astro`, `site/scripts/test-t14.mts`; `data/{stances,promises,changelog}.json` (113 ankare + 21 arkiv). Pipeline 216 tester gröna, sajtsviten grön inkl. ny PDF-sidnummervakt.

## 2026-07-15 — Delfrågeformulering: mekanism-krav ("lagstadgas") sänkt till debattens höjd

**Beslut:** Två delfrågor i äldreomsorgen band beskedet vid en specifik mekanism (lag) i stället för själva sakfrågan, vilket skapade falsk tystnad — partier som uttalat vill ha kravet men inte specificerar HUR filtrerades bort. Omformulerade: `sq-aldre-sprakkrav` "Ska språkkrav i svenska för omsorgspersonal **lagstadgas**?" → "Ska det ställas krav på kunskaper i svenska för personal i äldreomsorgen?"; `sq-aldre-bemanning` "Ska en nationell bemanningsnorm för äldreomsorgen **lagstadgas**?" → "Ska det införas en nationell bemanningsnorm för äldreomsorgen?". Testet som skiljer: kan målet nås ENBART via lag? JA (vinstförbud, kärnvapenförbud — där ÄR målet en lagstiftningsakt) → mekanism-ordet ok. NEJ (språkkrav/bemanningsnorm kan ske via lag, föreskrift, statsbidrag, arbetsgivarkrav) → mekanism-ordet är för snävt och döljer besked. Granskning av alla 22 delfrågor: exakt dessa två träffades; de tre kvarvarande "i lag"-frågorna (sjukvård-vinst, skola-vinst, försvar-kärnvapen) är oförändrade eftersom de har publicerade besked som bevisar att de triggar. Verify-regeln (beskedet måste följa ur citatet ensamt) ändras INTE — vi tolkar inte citaten friare, vi ställer rätt fråga så att citat på ämnet verifierar rent. De fyra köade språkkravsposterna, som alla fällts av LLM B med motiveringen "säger inget om kraven ska lagstadgas", kördes mot den nya formuleringen: C (Valmanifest s. 42) och S (Valplattform s. 3) godkändes som JA (st-2026-0022/0023, exakt sidankare + Wayback-arkiv via source-page-fix), två S-dubbletter från val-2026-sidan avvisades.
**Motiv:** Kärnlöftet är att ingen ska kunna gömma sig — men här var det motsatsen: fyra partier HADE uttalat sig om språkkrav och sajten visade "besked saknas". Novus m.fl. formulerar inte ja/nej-frågor alls (deras "viktigaste fråga" används bara för urvalet), så "lagstadgas"-precisionen var vårt eget påfund utan extern förankring — fritt att korrigera. Bemanningsfrågan hade samma latenta fel (cellen helt tom); rättad förebyggande innan ett filtrerat besked hann uppstå.
**Förkastade alternativ:** Tolka citaten friare i verify-steget (hade brutit neutraliteten — så smugglas ståndpunkter in som partiet inte tagit); låta mekanismen ligga kvar i huvudfrågan (döljer besked); ta bort delfrågorna (frågorna är reella valfrågor). Mekanismen (lag vs annat) kan bli en EGEN delfråga den dag ett parti faktiskt kampanjar på lagstiftning.
**Påverkan:** `data/issues.json` (två formuleringar + fairness_notes), `data/stances.json` (st-2026-0022 C, st-2026-0023 S), `data/stances_review.json` (4 språkkravsposter ur kön). Pipeline 216 tester gröna, sajtsviten grön, RS1–RS5 gröna. Sprakkrav-cellen: 2 av 8 partier har nu besked.

## 2026-07-15 — Straffålder: C godkänd, V ersatt med fotnotsfritt citat ur motion

**Beslut:** Straffålder-frågan ("Ska straffmyndighetsåldern sänkas till 14 år för allvarliga brott?") är väl formulerad — inget mekanism-problem som i äldreomsorgen. De två köposterna hade var sitt eget, mindre fel: (1) C:s besked var en ren, ordagrann NEJ som direkt bemötte 14-års-förslaget ("vi vidhåller detta även när nu förslaget är att straffbarhetsåldern ska vara 14 år", riksdagsdok HD024228) och väntade bara på granskning (MODE) — godkänd (st-2026-0024). (2) V:s köade citat föll korrekt på G3: den enda meningen där V uttryckte sin egen hållning i det dokumentet (HD024230) bar en fotnotsmarkör "[2]" och extraktionen tappade dessutom mellanslaget före den. På ägarens uppmaning söktes ett renare citat: V:s motion mot just prop. 2025/26:293 (HD024211, 2026-07-07) innehåller den fotnotsfria meningen "Riksdagen bör avslå regeringens förslag i de delar det avser en sänkning av straffbarhetsåldern till 14 år." Publicerad som st-2026-0025 via engångsskriptet add-v-straffalder.mts, som INTE tar någon genväg förbi verbatimkedjan: kandidaten kördes genom de riktiga grindarna (runStanceGates — G2/G3/G6/G7 mot den hämtade källtexten) och publicerades bara sedan G3 verbatim passerat, via publishStances med humanApproved. Gamla V-posten avvisad som ersatt. Straffålder-cellen: 3 av 8 partier (C, V, MP) har nu tydligt nej.
**Motiv:** Ägarfråga "är det en svår fråga igen från vår sida?" — nej, formuleringen var god; problemen låg i källorna, inte i frågan. V:s position (emot sänkt straffbarhetsålder) är otvetydig; att låta ett fotnotsartefakt-citat blockera beskedet vore fel, men lika fel vore att publicera ett citat med "[2]" som ser ut som ett fel för läsaren, eller att mjuka upp verbatim-grinden. Rätt lösning: hämta det renare citatet ur V:s egen motion mot exakt det förslag frågan gäller — starkast möjliga källa.
**Förkastade alternativ:** Godkänna det fotnotsbärande V-citatet via CLI (omöjligt — G3 är hård grind, och rätt så); publicera "[2]" verbatim (ser ut som fel); handknäcka in beskedet i stances.json utan grindkörning (bryter verbatimkedjans integritet — hela poängen är att G3 faktiskt prövas).
**Påverkan:** `pipeline/scripts/add-v-straffalder.mts` (nytt engångsskript), `data/stances.json` (st-2026-0024 C, st-2026-0025 V), `data/stances_review.json` (2 straffålderposter ur kön). Pipeline 216 tester gröna, typecheck grön, RS1–RS5 gröna.

## 2026-07-15 — Kön tömd: S värnplikt + SD matmoms publicerade ur skarpare källor

**Beslut:** De två sista köposterna löstes genom att hämta skarpare källor, inte genom att mjuka upp grindarna. (1) S · värnplikt: köcitatet ("fler värnpliktiga") klarade inte frågans medvetet precisa ribba "utöver redan beslutade nivåer" — och den precisionen är själva poängen (en upptrappning är redan beslutad, så bara "fler" skiljer inte partierna). S:s nyhet 2025-10-30 säger uttryckligen "målet för antalet värnpliktiga höjs till 12 000 till år 2030, vilket är 2 000 fler än nuvarande mål" — den "2 000 fler" är precis det utöver-beslutat som saknades. Publicerad som st-2026-0026 (ja). (2) SD · matmoms: köcitatet var ett fragment ("fortsatt låga skatter på … livsmedel"). SD:s valplattform 2026 säger rakt ut "Den nedsatta momsen på mat ska bli permanent, primärt för sådant som är basvaror och som kan produceras i Sverige" — direkt svar på om den tillfälliga sänkningen ska bli permanent. Publicerad som st-2026-0027 (ja), PDF s. 9. Båda kördes genom de riktiga grindarna (add-stances-manual.mts → runStanceGates G2/G3/G6/G7 mot hämtad källtext; SD via pdfPages så exakt sidankare sattes) och publicerades via publishStances med humanApproved. Tre ersatta köposter avvisade. **Ståndpunktskön är nu tom.**
**Motiv:** Samma princip som straffålder: när en köpost är för vag/fragmentarisk är rätt åtgärd att leta upp ett skarpare, ordagrant citat ur partiets egen publicerade källa — aldrig att sänka verify-ribban eller tolka ett svagt citat välvilligt. Värnpliktsfrågans precision ("utöver beslutade nivåer") är korrekt och skiljer sig från språkkravsfallet: där dolde mekanism-kravet beskedet, här är precisionen det enda som gör frågan meningsfull.
**Förkastade alternativ:** Godkänna S:s MODE-köpost ("fler värnpliktiga") trots att den inte klarar frågans ribba (hade publicerat ett "ja" som inte följer ur citatet mot just den frågan); godkänna SD-fragmentet (för svagt — nämner inte permanent).
**Påverkan:** `pipeline/scripts/add-stances-manual.mts` (nytt), `data/stances.json` (st-2026-0026 S, st-2026-0027 SD), `data/stances_review.json` (nu tom). S:s och V:s Wayback-arkiv backfillas när SPN2 släpper (dokumenterad lucka; källorna live). Pipeline 216 tester gröna, sajtsviten grön, RS1–RS5 gröna.

## 2026-07-15 — Förklaring till tomma celler + arkiveringsalternativ + riksdagsskörd (utfall)

**Beslut:** (1) Ny delad komponent `TomcellsNot.astro` visas överst på /fragor och på varje /fraga-sida med minst en tom cell. Den förklarar att "besked saknas" betyder att vi ännu inte hittat ett ordagrant publicerat uttalande — inte att partiet saknar åsikt eller gömmer sig — och särskilt att FÄRRE besked kan bero på att partiets källor varit svårare att hitta, inte på att partiet är mer svävande. Skyddar neutraliteten mot att ojämn täckning (S 9, M 3) läses som en värdering. (2) Arkiveringsutredning: Wayback är primärt men spärrar vissa URL:er (robots/SPN2-rate). archive.today ger 429 genom vår proxy (kan funka från Actions men opålitligt). Slutsats: Wayback primärt; för det Wayback vägrar är sajtens egen rot-check (`source_status` ok/andrad/borttagen) + publikt git-historik integritetsgarantin — vi upptäcker om källan ändras även utan tredjepartskopia. Tyngre alternativ (Perma.cc, OpenTimestamps) noterade men ej införda. (3) Riksdagsmotion-skörd testad men gav noll nya celler: regeringspartierna (M/KD/L/SD) motionerar inte mot egna propositioner (onåbara den vägen), och oppositionens motioner är mest redan täckta via A-Ö eller formulerar sig vagt kring de specifika frågorna ("justera tak", "förändrat takbelopp" — inte "höj taket"). Mekanismen validerad, men låg marginalnytta för de tomma cellerna.
**Motiv:** Med 27% täckning ser besökaren mest "besked saknas"; utan kontext hotar det både trovärdigheten och neutraliteten. Arkivfrågan väcktes av ägaren; svaret är att integritet inte kräver Wayback specifikt — verbatim + rot-check + öppen kod bär den.
**Påverkan:** `site/src/components/TomcellsNot.astro` (ny), `site/src/pages/{fragor,fraga/[slug]}.astro`; `data/stances.json` (7 Wayback-arkiv backfillade, 51/52 nu arkiverade — endast C:s utbildningssida kvar, dokumenterad lucka). Sajtsvit grön, RS gröna.

## 2026-07-16 — Arkiv-fallback: archive.today när Wayback vägrar

**Beslut:** Arkivkedjan är nu Wayback → archive.today (bägge kontofria, oberoende). `pipeline/src/archive.ts` fick `archiveViaArchiveToday` + `archiveWithFallback` (Wayback primärt; för det Wayback spärrar via robots/rate provas archive.today), och `createArchiveFn` (som hela pipelinen använder) kör kedjan automatiskt. `extractArchiveTodayUrl` plockar färdig ögonblicksbild ur res.url/Location/Refresh och avvisar åtgärdssidor (submit/newest) och pågående arkivering (wip) — vi lagrar bara en stabil, färdig kopia. source-page-fix.mts fick samma fallback efter att Wayback (availability+save) misslyckats. retry=true bara om BÅDA misslyckas — då bär rot-checken + öppen git-historik integriteten tills nästa körning.
**Motiv:** Ägarfråga: fallback utan konto för det Wayback inte tar. Wayback spärrar vissa partisidor (t.ex. centerpartiet.se/utbildning); archive.today är den självklara kontofria kompletteringen. archive.today ger 429 från vår datacenter-proxy och går därför inte att liveköra här, men fungerar typiskt från GitHub Actions — därför verifieras URL-parsning och kedjelogik med 11 enhetstester (mock-fetch) i stället, och den skarpa körningen bekräftades degradera snyggt (Wayback-fail → archive.today-fallback → 429 → 'arkiv saknas', ingen krasch).
**Förkastade alternativ:** Perma.cc (kräver konto/kvot), OpenTimestamps (bevisar existens men ingen browsbar kopia), egen självhostad snapshot (mindre trovärdig — kan anklagas för manipulation). archive.today valdes för att den är kontofri, oberoende och ger en browsbar tredjepartskopia.
**Påverkan:** `pipeline/src/archive.ts` (fallback-kedja, createArchiveFn använder den), `pipeline/tests/archive.test.ts` (ny, 11 tester), `pipeline/scripts/source-page-fix.mts` (fallback i backfill), `pipeline/tests/fetch.test.ts` (Wayback-testerna pekar nu direkt på archiveViaWayback). Pipeline 227 tester gröna, typecheck grön. C:s utbildningssida (st-2026-0052) förblir tills vidare oarkiverad här (bägge tjänster spärrar från denna miljö) men fallbacken tar den från Actions.
...
**Beslut:** Genomgång av live-sajten med utomstående ögon inför extern spridning gav tre fynd, alla åtgärdade: (1) **Arkiv som inte backar citatet.** 4 av 25 HTML-arkiv (S sjukvård-vinst, S kärnkraft, C kärnkraft, KD värnplikt) var Wayback-kopior ÄLDRE än sidinnehållet — availability-API:t ger "närmaste" snapshot, inte en som täcker dagens text. Ny regel i hela kedjan: ett arkiv accepteras ENDAST om citatet står ordagrant i själva snapshotten (`src/archive-verify.ts` → `snapshotBacksQuote`, inkopplad i rot-checkens backfill och source-page-fix; engångsscript `fix-stale-archives.mts` nollställde de fyra — färska saves rate-limitades vid körning, rot-watchen tar dem med verifiering). Hellre ärligt tomt än ett arkiv som inte bevisar något. (2) **Två olika gap-siffror.** Veckokrönikan sade 12 978 mdkr där startsidan sade 7 864: krönikans totalFlasket saknade grupp-dedup (R3) och aktiv-filter, och gap var satt = total (gammal definition). Nu räknar krönikan EXAKT som startsidan (dedup + aktiva + utgift/intäktsminskning; gap = total − reformbudget ur constants.json, ny obligatorisk parameter). v28-posten rättad i data (rubrik, gap-mening, fälten). (3) **Rubriken "skolavskaffande".** MP:s löfte är att avskaffa MARKNADSSKOLAN; krönikerubrikens komprimering lät som att skolan skulle avskaffas. Rättad till "avskaffad marknadsskola" + ny RUBRIKREGEL i A4-prompten: löftets egna nyckelord får aldrig komprimeras så innebörden ändras. **Bonusfynd:** rot-checkens PDF-gren läste `.text` på PdfExtract som bara har `pages` — undefined hade kraschat hela körningen på första PDF-källan (26 av 52 statements) första måndagen; rättad till `pages.join`.
**Motiv:** Alla tre fynd är exakt vad en skeptisk förstaläsare (journalist, partimänniska) hittar först: arkivlänkar som inte bevisar citatet, två motstridiga huvudsiffror, en rubrik som missrepresenterar ett parti. Sajtens hela värde är att sådant inte finns.
**Förkastade alternativ:** Behålla icke-backande arkiv "tills bättre finns" (värre än tomt — ser ut som bevis men är det inte); låta krönikans gamla gap-definition stå kvar med fotnot (två definitioner av samma ord på samma sajt är oförsvarbart); enbart rätta v28-rubriken utan promptregel (felet hade återkommit nästa vecka).
**Påverkan:** `pipeline/src/{chronicle,index,archive-verify}.ts`, `pipeline/prompts/A4-weekly.md`, `pipeline/scripts/{source-rot-check,source-page-fix,fix-stale-archives}.mts`, `pipeline/tests/chronicle.test.ts`, `data/chronicles.json` (v28 rättad), `data/stances.json` (4 arkiv nollställda i väntan på verifierad kopia). Pipeline 228 tester gröna, sajtsvit grön; krönika och startsida visar nu samma tal (8 184 / 320 / 7 864).

## 2026-07-17 — Andra genomgången: rotorsaken till .text-buggen stängd + tidskorrekta krönikerättelser

**Beslut:** Ägarens uppmaning "kolla igenom allt en gång till och fixa så det inte sker igen" gav fyra åtgärder. (1) **Rotorsak funnen:** tsconfig typecheckade bara src/ och tests/ — scripts/*.mts (rot-check, review-CLI, backfills) kompilerades aldrig, vilket är exakt varför `.text`-buggen överlevde till main. Nu inkluderas scripts/ + facit/ i typecheck (0 nya fel efter fixen — buggen var den enda som gömde sig, men kategorin är stängd i CI). (2) **v27 hade samma gap-bugg** (gap=total=13 059) — rättad (gap 12 739). Viktig nyans: v27:s total var KORREKT för sin tid (inga grupper fanns vid seedningen), och min första v28-rättelse var fel-i-princip: den använde dagens tal (8 184/7 864) i stället för genereringsveckans. En krönika är en veckoögonblicksbild — v28 bär nu talen per 2026-07-09, beräknade ur promises.json vid exakt den commiten med nya formeln (fläsk 12 057, gap 11 737), tidsförankrat i texten ("Vid veckans utgång…"). Skillnaden mot dagens startsida (8 184) är hederlig tidsdrift (tvärparti-grupperingen infördes efter 07-09), inte inkonsekvens. (3) **Ny datavakt:** chronicle.test.ts verifierar mot verkliga chronicles.json att VARJE post uppfyller gap = max(0, total − reformbudget) — "gap=total"-klassen kan aldrig publiceras tyst igen. (4) **Sajt-paritet:** sajtens totalFlasket/totalBesparingar filtrerar nu aktiva löften precis som pipelinens chronicle-formel (0 skillnad idag — inga tillbakadragna finns), så en framtida tillbakadragning inte splittrar talen. PDF-arkivens innehållsverifiering (26 citat) pågår i bakgrunden i snäll takt — Wayback rate-limitar hårt efter dagens många anrop; auditens metod är densamma som HTML-genomgången.
**Förkastade alternativ:** Rätta v28 till dagens tal (fel-i-princip — ögonblicksbilder ska bära sin tids tal); statisk regel "arkiv-timestamp ≥ date_stated" (falskpositiv för oförändrade sidor med äldre giltig snapshot — innehållsverifieringen är den riktiga invarianten).
**Påverkan:** `pipeline/tsconfig.json`, `data/chronicles.json` (v27+v28), `pipeline/tests/chronicle.test.ts` (datavakt), `site/src/lib/aggregates.ts`. Pipeline 229 tester gröna (inkl. datavakten mot verklig data), typecheck grön inkl. scripts, sajtsvit grön; "skolavskaffande" förekommer inte längre någonstans i dist.

## 2026-07-17 — Rättelsesystemet gjort verkligt: synliga noter på rättade krönikor + rättelselogg som data
