...
const englishPartyNameTerms: Record<string, string[]> = {
...
function selectedEnglishPartyNameTerms(codes?: string[]): Set<string> {
  return new Set((codes ?? []).flatMap((code) => englishPartyNameTerms[code] ?? []));
}

function queryTermGroups(query?: string, partyCodes?: string[]): string[][] {
...
function queryTerms(query?: string, partyCodes?: string[]): string[] {
  return queryTermGroups(query, partyCodes).flat();
}

function matchesQuery(query: string | undefined, partyCodes: string[] | undefined, ...fields: string[]): boolean {
...
function evidenceList(evidence: Evidence[]): HTMLOListElement {
  const list = el("ol");
...
function researchBriefUrl(input: BriefInput): string {
...
async function searchEvidence(input: SearchInput) {
