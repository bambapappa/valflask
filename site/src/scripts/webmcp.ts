...
type Evidence = { kind: "lofte" | "besked"; title: string; party_codes: string[]; quote: string; date: string; source: Source; page_url: string; detail: string };
...
async function searchEvidence(input: { party_codes?: string[]; category?: string; kind?: "loften" | "besked" | "alla"; max_results?: number }) {
  ...
  const [promises, stances, issuesResponse, integrity] = await Promise.all([
    getJson<PromiseItem[]>("/api/v1/promises.json"), getJson<StanceCell[]>("/api/v1/stances.json"),
    getJson<{ issues: Issue[] }>("/api/v1/issues.json"), getJson<{ data_hash: string }>("/api/v1/integrity.json"),
  ]);
...
async function showPartyComparison(input: { party_codes: string[] }) {
  ...
  const summary = await getJson<{ data_hash: string; parties: Array<{ code: string; name: string; total_msek: number; promises_count: number; financing_gap_msek: number }> }>("/api/v1/summary.json");
