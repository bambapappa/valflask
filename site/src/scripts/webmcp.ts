...
async function searchEvidence(input: { party_codes?: string[]; category?: string; kind?: "loften" | "besked" | "alla"; max_results?: number; require_archive_copy?: boolean }) {
  ...
  const [promisesResponse, stancesResponse, issuesResponse, integrity] = await Promise.all([
    getJson<{ data: PromiseItem[] }>("/api/v1/promises.json"), getJson<{ stances: StanceCell[] }>("/api/v1/stances.json"),
