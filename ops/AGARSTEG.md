...
| Branch protection (ruleset) | **Boten behöver undantag** — pipelinen pushar data-commits direkt till `main` via `GITHUB_TOKEN` (`github-actions[bot]`), och skyddsregler gäller även den. Gör så: Repo → Settings → Rules → Rulesets → **New branch ruleset**: namn `main-skydd`, Enforcement **Active**, Target: default branch. Regler: ✓ Restrict deletions · ✓ Block force pushes · ✓ Require a pull request before merging (0 required approvals — du är ensam och kan inte godkänna egna PR:ar; PR-tvånget räcker per SPEC §14). **Bypass list: Add bypass → appen "GitHub Actions" → Always** — då får workflows pusha direkt medan människor går via PR. Inget bypass på force-push för någon. *(Syns inte GitHub Actions-appen i bypass-väljaren: använd klassisk regel under Settings → Branches med "Allow specified actors to bypass required pull requests" → `github-actions`.)* |
| Dependabot | PR:ar med **major**-bumpar (t.ex. Astro 5→6) mergeas inte under valrörelsen — stäng eller låt ligga; `.github/dependabot.yml` ignorerar nu majors för runtime-beroenden (§14: inga beroendebyten utan DECISION_LOG-rad). Patch-/minor- och säkerhetsuppdateringar är OK att merga när CI är grön |
